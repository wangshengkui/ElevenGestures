	...
	else
	{
		...
		else if (gestureName[0] == '-')
		{
			if (gestureName[1] >= '1' && gestureName[1] <= '9')
			{
				...
				{
					if (gestureName[1] == '1' && gestureName[2] == '0')
					{
						label = 10;
						return true;
					}
...
void SaveCollectData()
{
	...
	if (res.size() > 0 || firstGesture.size() > 0)
	{
		if (bihua == 3)
		{
			outfile.open("11指令data\\11指令collectData\\11指令collectDataStroke3.txt", ios::app);
		}
		else
		{
			outfile.open("11指令data\\11指令collectData\\11指令collectDataStroke12.txt", ios::app);
		}
		outfile << label << " ";
...
void writeToPoint()
{
	...
	if (res.size() > 0 || firstGesture.size() > 0)
	{
		outfile.open("11指令data\\point\\point.txt");
		outfile << tag << " ";
...
void calCharacter()
{
	...
	infile.open("11指令data\\point\\point.txt");
	outfile.open("11指令data\\point\\pointCharacter.txt");
...
void PredictGesture()
{
	...
	char *tmp[4] = { "" };
	tmp[1] = { "11指令data\\point\\pointCharacter.txt" };
	if (bihua == 3)
	{
		tmp[2] = { "11指令data\\11指令model\\11指令modelStroke3.txt" };
	}
	else
	{
		tmp[2] = { "11指令data\\11指令model\\11指令modelStroke12.txt" };
	}
	tmp[3] = { "11指令data\\point\\outPoint.txt" };
...
void printResult()
{
	...
	infile.open("11指令data\\point\\outPoint.txt");
	getline(infile, s);
	infile.close();
...
void trainModel()
{
	cout << "********************模型训练********************" << endl;
	sampleCount = 0;
	MergeTrainFile("11指令data\\11指令temp\\11指令tempStroke12.txt","11指令data\\11指令collectData\\11指令collectDataStroke12.txt", "11指令data\\11指令train\\11指令trainStroke12.txt");
	MergeTrainFile("11指令data\\11指令temp\\11指令tempStroke3.txt", "11指令data\\11指令collectData\\11指令collectDataStroke3.txt", "11指令data\\11指令train\\11指令trainStroke3.txt");
	cout << "* 【开始计算特征】                             *" << endl;
	CalTrainDataCharac("11指令data\\11指令character\\11指令characterStroke12.txt", "11指令data\\11指令temp\\11指令tempStroke12.txt");
	CalTrainDataCharac("11指令data\\11指令character\\11指令characterStroke3.txt", "11指令data\\11指令temp\\11指令tempStroke3.txt");
	cout << "* 【特征计算完毕】                             *" << endl;
	...
	tmp[3] = { "-t" };
	tmp[4] = { "0" };
	tmp[5] = { "11指令data\\11指令character\\11指令characterStroke12.txt" };
	tmp[6] = { "11指令data\\11指令model\\11指令modelStroke12.txt" };
	cout << "* 【开始训练特模型，请稍后】                   *" << endl;
	svmTrain(paramNumberPredict, tmp);

	tmp[5] = { "11指令data\\11指令character\\11指令characterStroke3.txt" };
	tmp[6] = { "11指令data\\11指令model\\11指令modelStroke3.txt" };
	svmTrain(paramNumberPredict, tmp);
...
void LoadUsageRecord()
{
	...
	infile.open("11指令data\\11指令StatisticalResults\\11指令StatisticalResults.txt");
	...
	infile.open("11指令data\\11指令train\\11指令trainStroke12.txt");
	while (getline(infile, s))
	{
		sampleCount++;
	}
	...
	infile.open("11指令data\\11指令train\\11指令trainStroke3.txt");
	while (getline(infile, s))
	{
		sampleCount++;
	}
...
{
	remove("11指令data\\11指令StatisticalResults\\11指令StatisticalResults.txt");
	ofstream outfile;

	SYSTEMTIME st;
	GetLocalTime(&st);

	outfile.open("11指令data\\11指令StatisticalResults\\11指令StatisticalResults.txt",ios::app);
...
void MergerExternalDataFile(const string &sourceDataFile12, const string &sourceDataFile3, const string &sourceStatisticalFile)
{
	...
	string s;
	ofstream outfile;
	infile.open("11指令data\\11指令externalDataFile\\"+sourceDataFile12);
	outfile.open("11指令data\\11指令collectData\\11指令collectDataStroke12.txt", ios::app);
	...
	infile.open("11指令data\\11指令externalDataFile\\" + sourceDataFile3);
	outfile.open("11指令data\\11指令collectData\\11指令collectDataStroke3.txt", ios::app);
	...
	infile.open("11指令data\\11指令externalStatiscalResults\\" + sourceStatisticalFile);
...
{
	ifstream infile;
	infile.open("11指令data\\collectData.txt");
	string s;
		...
		if (nCin == 10)
		{
			...
			else
			{
				...
				else if (tempInput == "2")
				{
					...
					{
						cout << "【请输入手势正确标签(指令控制符：0，其余手势为-1~-10)】：";
						string gestureName;
						cin >> gestureName;
